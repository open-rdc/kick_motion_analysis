...
function [t1, t2, t1d, t2d] = dynamics(t1, t2, t1d, t2d, f1, f2, l1, l2, m1, m2, I1, I2, motor_velocity, dt)
  g = 9.8;
  M = [(l1^2*m1+4*l1^2*m2+4*I1)/4 (l1*l2*m2*cos(t2-t1))/2;
       (l1*l2*m2*cos(t2-t1))/2    (2*l2^2*m2+8*I2)/8];
  D = [((l1*l2*m2*t2d^2-l1*l2*m2*t1d*t2d)*sin(t2-t1))/2;
       ((l1*l2*m2*t1d*t2d-l1*l2*m2*t1d^2)*sin(t2-t1))/2];
  G = [g*l1*m2*cos(t1)+(g*l1*m1*cos(t1))/2;
       (g*l2*m2*cos(t2))/2];
  tdd = inv(M) * [[f1; f2] - D - G];
  t1dd = tdd(1);
  t2dd = tdd(2);
...
for t = 0.0: dt/period: 2 * period/period
  ...
  f1 = max([min([gain * (t1 - t1r), max_torque * (1 + w1r)]), -max_torque * (1 - w1r)]);
  w2r = -t2rd/motor_velocity;
  f2 = max([min([gain * (t2 - t2r), max_torque * (1 + w2r)]), -max_torque * (1 - w2r)]);
