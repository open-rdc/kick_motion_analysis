...
%parameter low
stroke = 0.10 % (m)
P = [0, 0; 0, 0.06; 0.20, 0.06; stroke, 0.0];
...
%  sup_pos = x0 * cosh(mod(t,1) * period / Tc) + Tc * v0 * sinh(mod(t,1) * period / Tc) - x0;
  ...
  elseif (t <= 0.60)
    ...
    f1 = p_gain * (t1 - t1r) - d_gain * t1rd;
    f2 = p_gain * (t2 - t2r) - d_gain * t2rd;
  endif

  w1r = -t1rd/motor_velocity;
  f1 = max([min([f1, max_torque * (1 + w1r)]), -max_torque * (1 - w1r)]);
  w2r = -t2rd/motor_velocity;
  f2 = max([min([f2, max_torque * (1 + w2r)]), -max_torque * (1 - w2r)]);
